...
class PriceService {
  constructor(mqttClient, config, logger = console, eventEmitter) {
    ...
    this.manualFeed = !!this.config.manualFeed;
  ...
  _subscribeToPriceTopic() {
    if (!this.mqttClient) {
      throw new Error('[PriceService] mqttClient is required when manualFeed is false.');
    }
  ...
  ingestPriceData(priceData, options = {}) {
    if (!priceData || typeof priceData !== 'object' || !priceData.priceDate || !Array.isArray(priceData.hourly)) {
      this.logger.error('[PriceService] ingestPriceData received invalid data. Skipping update.');
      return;
    }

    const existingIndex = this.twoDaysData.findIndex((entry) => entry && entry.priceDate === priceData.priceDate);
    if (existingIndex !== -1) {
      this.twoDaysData[existingIndex] = priceData;
      if (this.debug) this.logger.info(`[PriceService] Updated price data for ${priceData.priceDate} via manual feed.`);
    } else {
      this.twoDaysData.push(priceData);
      if (this.debug) this.logger.info(`[PriceService] Added price data for ${priceData.priceDate} via manual feed.`);
    }
    ...
    this.priceUpdateTimeout = setTimeout(() => this.processReceivedPrices(), delay);
  }
