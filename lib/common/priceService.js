const { skewDays, parseJsonSafely } = require("../../misc/util"); // Assuming util.js is in misc folder at root
  ...
  _subscribeToPriceTopic() {
    ...
    this.mqttClient.on("message", (msgTopic, message) => {
      if (msgTopic.startsWith(this.priceTopic)) {
        ...
        const result = parseJsonSafely(message); // Using imported parseJsonSafely
