...
class WebSocketServer {
        ...
        this.wss.on('connection', (client) => {
            ...
            client.on('message', (message) => {
                try {
                    ...
                    if (msg === 'init') {
                        this.logger.info(`[WebSocketServer] Request from client ${clientId}: ${JSON.stringify(data)}`);
                        const chartData = this.chartDataService.getChartDataForClient();
                        if (chartData) {
                            this.wsSend(clientId, clientId, 'full', chartData);
                        } else {
                            this.logger.warn(`[WebSocketServer] No chart data available for client ${clientId}`);
                        }
                    }
                    this.logger.info(`[WebSocketServer] Active clients: ${this.clients.getClientIds()}`);
                } catch (error) {
                    this.logger.error(`[WebSocketServer] Error processing message: ${error.message}`);
                }
            });

            client.on('close', (connection) => {
                this.logger.info(`[WebSocketServer] Connection closed for clientId: ${client.clientId}, connection: ${connection}`);
                if (client.clientId) {
                    this.clients.deleteClient(client.clientId);
                    this.logger.info(`[WebSocketServer] Client removed: ${client.clientId}. Active clients: ${this.clients.getClientIds()}`);
                }
            });

            client.on('error', (error) => {
                this.logger.error(`[WebSocketServer] Error on client ${client.clientId || 'unknown'}: ${error.message}`);
            });
        });

        this.logger.info(`[WebSocketServer] WebSocket server started on port ${this.wss.options.port}`);
    }
    ...
    wsSend(clientId, channel, topic, message) {
        ...
        try {
            ...
            this.logger.error(`[WebSocketServer] Error sending message to client ${clientId}: ${error.message}`);
        }
    }
        ...
        this.wss.clients.forEach((client) => {
            ...
            if (client.readyState === WebSocket.OPEN) {
                try {
                    ...
                    this.logger.error(`[WebSocketServer] Error sending message to all clients (client ${client.clientId || 'unknown'}): ${error.message}`);
                }
            }
        });
    }
