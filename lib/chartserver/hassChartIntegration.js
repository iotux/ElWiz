class HassChartIntegration {
  ...
  publishDiscoveryMessages() {
    if (!this.hassPublish) {
      this.logger.info(
        "[HassChartIntegration] Home Assistant publishing is disabled.",
      );
      return;
    }
    if (!this.mqttClient || !this.mqttClient.connected) {
      this.logger.warn(
        "[HassChartIntegration] MQTT client not connected, cannot publish HASS discovery messages.",
      );
      return;
    }
    ...
    try {
      ...
      if (this.debug) {
        this.logger.info(
          `[HassChartIntegration] HA announce thresholdLevel: ${JSON.stringify(announce)}`,
        );
      }
      ...
      if (this.debug) {
        this.logger.info(
          `[HassChartIntegration] HA announce spotBelowThreshold: ${JSON.stringify(announce)}`,
        );
      }
      this.logger.info(
...
module.exports = HassChartIntegration;
