    ...
    if (chartWasModified) {
      ...
      if (currentChartIndex !== -1) {
        ...
        if (previousCurrentHourIsBelowThreshold !== null && 
          ...
          // Publish the updated spotBelowThreshold as the status has changed
          if (this.mqttClient && this.mqttClient.connected) {
            this.mqttClient.publish(`${this.statTopic}/spotBelowThreshold`, String(newCurrentHourIsBelowThreshold), this.pubOpts);
            if (this.config.debug) {
              this.logger.info(`[ChartDataService] SpotBelowThreshold changed from ${previousCurrentHourIsBelowThreshold} to ${newCurrentHourIsBelowThreshold} after adjustment, republished.`);
            }
          }
        }
      }
