...
class ChartDataService {
  ...
  async processMqttChartAdjustment(topic, messagePayload) {
    ...
    // Get the current chart index and its isBelowThreshold value before the adjustment
    const currentChartIndex = this._getCurrentChartIndex();
    const previousCurrentHourIsBelowThreshold = currentChartIndex !== -1 ? this.chartData[currentChartIndex]?.isBelowThreshold : null;
    ...
    if (chartWasModified) {
      ...
      // Check if the current hour's isBelowThreshold status has changed after the adjustment
      if (currentChartIndex !== -1) {
        const newCurrentHourIsBelowThreshold = this.chartData[currentChartIndex]?.isBelowThreshold;
        if (previousCurrentHourIsBelowThreshold !== null && 
            previousCurrentHourIsBelowThreshold !== newCurrentHourIsBelowThreshold) {
          // Publish the updated data as the status has changed
          if (this.mqttClient && this.mqttClient.connected) {
            // Call publishData to update all relevant topics including spotBelowThreshold and thresholdLevel
            this.publishData(0, 'thresholdStatusChanged');
            if (this.config.debug) {
              this.logger.info(`[ChartDataService] SpotBelowThreshold changed from ${previousCurrentHourIsBelowThreshold} to ${newCurrentHourIsBelowThreshold} after adjustment, republished via publishData.`);
            }
          }
        }
      }
