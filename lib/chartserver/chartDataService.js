...
class ChartDataService {
    ...
    if (!fs.existsSync(this.savePath)) {
      ...
      try {
        ...
        if (
          ...
          this.logger.warn(
            `[ChartDataService] Invalid data structure in ${this.saveFile}. Reinitializing with default values.`,
          );
          fs.writeFileSync(this.saveFile, JSON.stringify(initialOffsets));
          return initialOffsets;
        }
        ...
        this.logger.error(
          `[ChartDataService] Error reading or parsing ${this.saveFile}: ${error}. Reinitializing with default values.`,
        );
        fs.writeFileSync(this.saveFile, JSON.stringify(initialOffsets));
        return initialOffsets;
      }
  ...
  saveThresholds(idx, threshold, where) {
    if (this.offsetFactors && this.offsetFactors[idx]) {
      ...
      if (this.config.debug) {
        this.logger.info(
          `[ChartDataService] saveThresholds ${where}: saved ${JSON.stringify(this.offsetFactors)}`,
        );
      }
  ...
  parseJsonSafely(message) {
    ...
    try {
      ...
      this.logger.error(
        "[ChartDataService] Error converting buffer to string:",
        err,
      );
      return {
        error: true,
        message: "Message cannot be parsed as string",
        data: null,
      };
    }
  ...
  processMqttPriceData(topic, messagePayload) {
    ...
    if (!result.error) {
      if (this.twoDaysData.length < 2) {
        ...
        if (this.config.debug) {
          this.logger.info(
            `[ChartDataService] Pricedata skipped ${result.data.priceDate}`,
          );
        }
      }
  ...
  processMqttChartAdjustment(topic, messagePayload) {
    ...
    if (adjustmentType === "adjustLeftAvgOffset") {
      ...
      this.leftAvgOffsetFactor =
        parsed === 0 ? 0 : this.leftAvgOffsetFactor + parsed * this.stepFactor;
      ...
      this.rightAvgOffsetFactor =
        parsed === 0 ? 0 : this.rightAvgOffsetFactor + parsed * this.stepFactor;
      this.saveThresholds(
        1,
        this.rightAvgOffsetFactor,
  ...
  handleMessages() {
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] handleMessages: twoDaysData length ${this.twoDaysData.length}`,
      );
    }
    ...
    if (this.twoDaysData.length > 1) {
      ...
      if (
        this.offsetFactors &&
        this.offsetFactors.length > 1 &&
        this.twoDaysData[1].priceDate > this.offsetFactors[1].date
      ) {
        this.offsetFactors.push({
          date: this.twoDaysData[1].priceDate,
          threshold: 0,
        });
    ...
    if (
      this.twoDaysData.length > 0 &&
      this.offsetFactors &&
      this.offsetFactors.length > 0
    ) {
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] AvgOffsetFactors: ${JSON.stringify(this.offsetFactors)}`,
      );
  ...
  updateChartData(prices, adjustment) {
    ...
    try {
      ...
      this.logger.warn(
        "[ChartDataService] No data to update in updateChartData.",
      );
      return;
    }
    ...
    if (this.chartData.length > 0) {
      currentMax = this.chartData.reduce(
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] updateChartData: maxPrice=${this.maxPrice}, avgPrice=${avgPrice}, fixed=${fixed}, adjustVal=${adjustVal}`,
      );
    }
    ...
    if (!hourlyData || hourlyData.length === 0) {
      if (this.config.debug)
        this.logger.info("[ChartDataService] No hourly data to update.");
      return;
    }
    ...
    if (!this.dataAvailable) {
      ...
      this.chartData = this.chartData.filter(
        (d) => d.startTime.slice(0, 10) !== newPriceDate,
      );
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] updateChartData processed. ChartData length: ${this.chartData.length}`,
      );
    }
  }
  ...
  async updateAvgData(startOffset, adjustment, from) {
    if (this.config.debug)
      this.logger.info(`[ChartDataService] updateAvgData invoked from ${from}`);
    ...
    if (!this.twoDaysData[dayIndex] || !this.twoDaysData[dayIndex].daily) {
      this.logger.warn(
        `[ChartDataService] updateAvgData: twoDaysData for dayIndex ${dayIndex} is not available.`,
      );
      return false;
    }
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] updateAvgData: maxPrice=${this.maxPrice}, avgPrice=${avgPrice}, fixed=${fixed}, adjustVal=${adjustVal}`,
      );
    }
    ...
    this.chartData.forEach((h, idx) => {
      if (idx >= startOffset && idx < startOffset + 24) {
        const newThresholdLevel =
          avgPrice > 0
            ? parseFloat((avgPrice + fixed + adjustVal).toFixed(3))
            : 0;
    ...
    if (
      this.chartData[startOffset] &&
      this.chartData[startOffset].startTime.slice(0, 10) === this.currentDate
    ) {
  ...
  publishData(startOffset, from) {
    ...
    if (!dataPoint) {
      if (this.config.debug)
        this.logger.warn(
          `[ChartDataService] publishData: No dataPoint for currentHour ${this.currentHour} + startOffset ${startOffset}`,
        );
      return;
    }
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] publishData invoked from ${from}, StartOffset: ${startOffset}\nchartDataPoint: ${JSON.stringify(dataPoint)}`,
      );
    }
    try {
      this.mqttClient.publish(
        `${this.statTopic}/spotPrice`,
        String(dataPoint.spotPrice),
        this.pubOpts,
      );
      this.mqttClient.publish(
        `${this.statTopic}/avgPrice`,
        String(dataPoint.avgPrice),
        this.pubOpts,
      );
      this.mqttClient.publish(
        `${this.statTopic}/thresholdLevel`,
        String(dataPoint.thresholdLevel),
        this.pubOpts,
      );
      this.mqttClient.publish(
        `${this.statTopic}/spotBelowThreshold`,
        String(dataPoint.isBelowThreshold),
        this.pubOpts,
      );
    } catch (err) {
  ...
  checkAndSendChartData = (() => {
    ...
    return (currentChartData) => {
      ...
      const currentStartTimes = currentChartData.map((data) =>
        data.startTime.slice(0, 10),
      );
      const uniqueStartTimes = new Set(currentStartTimes);
      const isNewData = [...uniqueStartTimes].some(
        (date) => !recentUpdates.has(date),
      );
      ...
      if (isNewData || currentChartData.length !== lastUpdateSize) {
        ...
        if (this.config.debug) {
          this.logger.info(
            "[ChartDataService] Data updated, should be sent to clients.",
          );
        }
        ...
        if (this.config.debug)
          this.logger.info(
            "[ChartDataService] No new data to send to clients.",
          );
  ...
  async performHourlyTasks() {
    ...
    if (this.chartData && this.chartData.length > 0) {
      this.isOnRightSide =
        this.chartData[0].startTime.slice(0, 10) === this.currentDate
          ? false
          : true;
    } else {
    ...
    if (!this.isVirgin && this.offsetFactors) {
      ...
      const currentFactor = this.isOnRightSide
        ...
        : this.offsetFactors[0]
          ? this.offsetFactors[0].threshold
          : 0;
      updated = await this.updateAvgData(
        startOffset,
        currentFactor,
        "hourlyTasks",
      );
    ...
    if (this.config.debug) {
      this.logger.info(
        `[ChartDataService] Hourly AvgOffsetFactors: ${JSON.stringify(this.offsetFactors)}`,
      );
      this.logger.info(
        `[ChartDataService] Hourly isOnRightSide: ${this.isOnRightSide}`,
      );
    }
