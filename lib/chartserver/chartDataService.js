...
class ChartDataService {
  ...
  async refreshChartDataFromPriceService() {
    ...
    // Initialize with default structure, potentially using current actual dates as fallbacks.
    // The skewDays calls here are fallbacks if priceService doesn't provide dates.
    let newFactor0 = { 
      date: psCurrentDate || (await this.skewDays(0)), 
      threshold: 0 
    };
    let newFactor1 = { 
      date: psNextDate || (await this.skewDays(1, newFactor0.date)), // Skew relative to factor 0's date if psNextDate is null
      threshold: 0 
    };
    
    // Ensure newFactor0.date is set (it should be by psCurrentDate or skewDays(0))
    // If psNextDate was null, and newFactor0.date was today, newFactor1.date will be tomorrow.

    // Preserve thresholds if dates match previous factors
    ...
    // Safeguard: If after all this, dates are identical (e.g. psCurrentDate is today, psNextDate is also today or null and fallback created today)
    // This also handles if psCurrentDate is null and psNextDate is null, newFactor0.date and newFactor1.date might become today and tomorrow.
    if (newFactor0.date && newFactor0.date === newFactor1.date) {
       ...
       newFactor1.date = await this.skewDays(1, new Date(newFactor0.date)); // Make it next day relative to factor 0
       newFactor1.threshold = 0; // Reset threshold as date has changed
       // Re-check threshold preservation for the *new* newFactor1.date (unlikely to match but for completeness)
       if (oldFactor1Date === newFactor1.date) {
           newFactor1.threshold = oldFactor1Threshold;
       } else if (oldFactor0Date === newFactor1.date) {
           newFactor1.threshold = oldFactor1Threshold;
    ...
    // Update local threshold convenience variables
    ...
    // Update isOnRightSide based on the most up-to-date currentPriceDate from priceService and chartData
    const today = this.getDateString(new Date()); // Current actual date
    this.isOnRightSide =
  ...
  async skewDays(days, baseDateInput = null) {
    ...
    // Ensure baseDateInput, if provided, is treated as a valid date string or null
    let baseTime;
    if (baseDateInput) {
    ...
    return this.getDateString(date); // getDateString remains unchanged
  }
