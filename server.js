...
mqttOpts.will = { topic: `${serverConfig.haBaseTopic || 'elwiz'}/chart/status`, payload: 'offline', retain: true, qos: 0 };
...
const webSocketServer = new WebSocketServer(serverConfig.wsServerPort || 8322, chartDataService, logger);
const hassIntegrationChart = new HassIntegrationChart(mqttClient, serverConfig, logger);
...
mqttClient.on('message', async (topic, message) => {
    ...
    if (topic.startsWith(priceTopicBase)) {
        ...
        setTimeout(() => {
            ...
            if (updated) {
                webSocketServer.wsSendAll('chart', 'update', chartDataService.getChartDataForClient());
            }
...
// --- MQTT Subscriptions ---
if (mqttClient.connected) { // Or use a connect event
    try {
        ...
        logger.info(`[Server] Subscribed to MQTT topics: ${priceTopic}/# and ${chartTopic}/#`);
    } catch (err) {
...
function scheduleHourlyTasks() {
    ...
    setTimeout(() => {
        logger.info('[Server] Performing initial run of hourly tasks post-startup.');
        runHourlyTasks(); 
...
function gracefulShutdown() {
    ...
    mqttClient.end(false, () => { // end(force=false, options, callback)
