...
async function getPrices(dayOffset) {
  ...
  if (missingPrice) {
    const url = `${nordPoolUrl}/${priceCurrency}/${await uriDate(dayOffset)}`;
      ...
      .then(function (body) {
        ...
        if (rows[0].Columns[priceRegion].Value !== "-") {
          for (let curHour = 0; curHour <= 23; curHour++) {
            ...
            const gridPrice =
              curHour >= parseInt(dayHoursStart) && curHour < parseInt(dayHoursEnd)
                ? gridDayHourPrice
                : gridNightHourPrice;
            let spotPrice =
              price.toString().replace(/ /g, "").replace(/(\d),/g, ".$1") / 100;
          ...
          let minPrice =
            rows[24].Columns[priceRegion].Value.toString()
              .replace(/ /g, "")
              .replace(/,/g, ".") * 0.001;
          let maxPrice =
            rows[25].Columns[priceRegion].Value.toString()
              .replace(/ /g, "")
              .replace(/,/g, ".") * 0.001;
          let avgPrice =
            rows[26].Columns[priceRegion].Value.toString()
              .replace(/ /g, "")
              .replace(/,/g, ".") * 0.001;
          let peakPrice =
            rows[27].Columns[priceRegion].Value.toString()
              .replace(/ /g, "")
              .replace(/,/g, ".") * 0.001;
          let offPeakPrice1 =
            rows[28].Columns[priceRegion].Value.toString()
              .replace(/ /g, "")
              .replace(/,/g, ".") * 0.001;
          let offPeakPrice2 =
            rows[29].Columns[priceRegion].Value.toString()
              .replace(/ /g, "")
              .replace(/,/g, ".") * 0.001;
          //console.log('Averages before', peakPrice, offPeakPrice1, offPeakPrice2);
          if (typeof peakPrice !== "number")
            peakPrice = parseFloat(
              calcAvg(6, 22, oneDayPrices.hourly).toFixed(4)
            );
          if (typeof offPeakPrice1 !== "number")
            offPeakPrice1 = parseFloat(
              calcAvg(0, 6, oneDayPrices.hourly).toFixed(4)
            );
          if (typeof offPeakPrice2 !== "number")
            offPeakPrice2 = parseFloat(
              calcAvg(22, 24, oneDayPrices.hourly).toFixed(4)
            );
          //console.log('Averages after', peakPrice, offPeakPrice1, offPeakPrice2);

          oneDayPrices.daily = {
            minPrice: parseFloat(
              (minPrice += (minPrice * spotVatPercent) / 100).toFixed(4)
            ),
            maxPrice: parseFloat(
              (maxPrice += (maxPrice * spotVatPercent) / 100).toFixed(4)
            ),
            avgPrice: parseFloat(
              (avgPrice += (avgPrice * spotVatPercent) / 100).toFixed(4)
            ),
            peakPrice: parseFloat(
              (peakPrice += (peakPrice * spotVatPercent) / 100).toFixed(4)
            ),
            offPeakPrice1: parseFloat(
              (offPeakPrice1 += (offPeakPrice1 * spotVatPercent) / 100).toFixed(
                4
              )
            ),
            offPeakPrice2: parseFloat(
              (offPeakPrice2 += (offPeakPrice2 * spotVatPercent) / 100).toFixed(
                4
              )
            ),
          ...
          console.log(
            programName + ": Day ahead prices are not ready:",
            priceDate
          );
        }
      })
...
async function publishMqtt(priceDate, priceObject) {
  ...
  try {
    if (priceObject === null) {
      ...
      console.log(
        programName + ": MQTT message removed:",
        pricePrefix + priceDate
      );
    } else {
      ...
      console.log(
        programName + ": MQTT message published:",
        pricePrefix + priceDate
      );
    }
  } catch (err) {
