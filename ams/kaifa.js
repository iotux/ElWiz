...
try {
  ...
  console.error(
    `[Kaifa] Error loading config file ${configFile}: ${error.message}`,
  );
...
const listDecode = async function (msg) {
  ...
  if (!isHourStarted && minuteIndex === 0) {
    ...
    if (hourIndex === 0) {
      obj.isDayStart = true;
      if (ts.substring(8, 10) === "01") obj.isMonthStart = true;
    }
  }
  ...
  if (elements >= 9) {
    ...
    obj.powerProductionReactive =
      hex2Dec(msg.substring(index + 30, index + 38)) / 1000;
  }
  ...
  if (elements === 13 || elements === 18) {
    ...
    if (obj.voltagePhase2 === 0) {
      obj.voltagePhase2 =
        Math.sqrt(
          (obj.voltagePhase1 - obj.voltagePhase3 * 0.5) ** 2 +
            (obj.voltagePhase3 * 0.866) ** 2,
        ).toFixed(0) * 1;
    }
  }
  ...
  if (elements === 14 || elements === 18) {
    ...
    obj.lastMeterProduction =
      hex2Dec(msg.substring(index + 10, index + 18)) / 1000;
    obj.lastMeterConsumptionReactive =
      hex2Dec(msg.substring(index + 20, index + 28)) / 1000;
    obj.lastMeterProductionReactive =
      hex2Dec(msg.substring(index + 30, index + 38)) / 1000;
